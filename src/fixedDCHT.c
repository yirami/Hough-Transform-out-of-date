 ...
 * All rights reserved.
 * 
 * Filename: fixedDCHT.c (compiled into mex)
 ...
 *			[H,theta,rho] = fixedDCHT(img,sizeRaw,relatInfo)
 * 
 * Author��Yirami
 * A-Description:
...
void fixedDCHT(unsigned int *img, unsigned int *vote, size_t m, size_t n, double rho_step, double *sinMap, double *cosMap, unsigned int thetaQ, unsigned int rhoQ, size_t rTop, size_t cLeft)
{
...
// --> Parameters check
    ...
    if(nrhs!=3) 
	{
        mexErrMsgIdAndTxt("MyToolbox:fixedDCHT:nrhs","Three inputs required.");
    }
    if(nlhs!=3) 
	{
        mexErrMsgIdAndTxt("MyToolbox:fixedDCHT:nlhs","Three outputs required.");
    }
    // check the type of input argument(s)
    if( !mxIsUint32(prhs[0]) ) 
	{
        mexErrMsgIdAndTxt("MyToolbox:fixedDCHT:notUint32","Input matrix must be type Uint32.");
    }
    // check the shape of input argument(s)
    if(mxGetNumberOfDimensions(prhs[0])!=2) 
	{
        mexErrMsgIdAndTxt("MyToolbox:fixedDCHT:not2D","Input must be 2-D.");
    }
// --> Accept input parameters
    ...
    if(mrows<3||ncols<3) 
	{
        mexErrMsgIdAndTxt("MyToolbox:fixedDCHT:toosmall","Input matrix must be larger than 3x3.");
    }
...
// --> Parameters can be added to function interface
	...
	{
        mexErrMsgIdAndTxt("MyToolbox:fixedDCHT:meaningless","Resolution of Theta or Rho is meaningless.");
    }
// --> Prepare data
...
// --> Main computation
    ...
    fixedDCHT(img,H,mrows,ncols,rho_step,sinMap,cosMap,thetaQ,rhoQ,rTop,cLeft);
// --> Rest
