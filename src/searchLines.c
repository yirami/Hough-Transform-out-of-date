 ...
 * All rights reserved.
 * 
 * Filename: searchLines.c (compiled into mex)
 ...
 *			[linesInfo] = searchLines(img,vote,thetaSeq,rhoSeq,
 *				... linesMax,lineGap)
 * 
 * Author：Yirami
 * A-Description:
...
void peaks(unsigned int *vote, size_t m, size_t n, unsigned int linesMax, double *thetaSeq, double*rhoSeq, double thetaStep,double rhoStep, double *peaksInfo)
{
	// built-in parameters
...
// --> Parameters check
    ...
    if(nrhs!=6) 
	{
        mexErrMsgIdAndTxt("MyToolbox:searchLines:nrhs","Six inputs required.");
    }
    if(nlhs!=1) 
	{
        mexErrMsgIdAndTxt("MyToolbox:searchLines:nlhs","One output required.");
    }
    // check the type of input argument(s)
    if( !mxIsUint32(prhs[0]) ) 
	{
        mexErrMsgIdAndTxt("MyToolbox:searchLines:notUint32","Input matrix of image must be type Uint32.");
    }
    if( !mxIsUint32(prhs[1]) ) 
	{
        mexErrMsgIdAndTxt("MyToolbox:searchLines:notUint32","Input matrix of voting space must be type Uint32.");
    }
    // check the shape of input argument(s)
    if(mxGetNumberOfDimensions(prhs[0])!=2) 
	{
        mexErrMsgIdAndTxt("MyToolbox:searchLines:not2D","Input of image must be 2-D.");
    }
    if(mxGetNumberOfDimensions(prhs[1])!=2) 
	{
        mexErrMsgIdAndTxt("MyToolbox:searchLines:not2D","Input of voting space must be 2-D.");
    }
...
// --> Main computation
    ...
    peaks(vote,mrowsVote,ncolsVote,linesMax,thetaSeq,rhoSeq,thetaStep,rhoStep,peaksInfo);
